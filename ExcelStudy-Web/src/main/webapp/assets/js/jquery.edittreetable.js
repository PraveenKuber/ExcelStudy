...
(function($){
	$.fn.bstreetable = function(options){
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-primary btn-sm j-addClass'><i class='fa fa-level-down'></i>&nbsp;Add</button>")).append($("<span>&nbsp;</span>")).append($("<button class='btn btn-primary btn-sm'>&nbsp;Expand</button>")).append($("<span>&nbsp;</span>")).append($("<button class='btn btn-primary btn-sm j-collapse'>&nbsp;Collapse</button>"));
        ...
        element.delegate(".j-collapse","click",function(){
        ...
        function renderHeader(_dom_header){
        	...
        	dom_row.append($("<span style='width:6%'></span>"));
        	//render extfield
    		for(var j=0;j<settings.extfield.length;j++){
    			var column = settings.extfield[j];    			
    			$("<span></span>").css("min-width","115px").text(column.title).appendTo(dom_row);
    		} 
        ...
        function generateColumn(row,extfield){
        	...
        	switch(extfield.type){
        		case "input5":generatedCol=$("<input type='text' class='form-control input-sm' size='5'/>").val(row[extfield.key]).attr("data-oldval",row[extfield.key]).attr("name",extfield.key);break;
                        case "input10":generatedCol=$("<input type='text' class='form-control input-sm' size='10'/>").val(row[extfield.key]).attr("data-oldval",row[extfield.key]).attr("name",extfield.key);break;
